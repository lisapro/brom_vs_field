...
def plot(self,opt = 'all'): 
    ...
    if opt == 'one':
        ...
        self.ax20 = self.figure.add_subplot(gs[0])  
        self.ax20.set_title(index +', ' + data_units)          
        self.ax20.yaxis.grid(True,'major')     
        self.ax20.set_ylabel('depth, cm', 
                        fontsize= self.font_txt)       
        self.ax20.set_ylim(self.ysedmax, self.ysedmin -30 ) 
    
        self.ax20.axhspan(self.ysedmin -30,0,
                     color='#c5d8e3',alpha = 0.4,
                     label = "bbl"  )        
        self.ax20.axhspan(self.ysedmax,0,
                     color='#b08b52',alpha = 0.4,
                     label = "sediment"  )        
        #if  self.change_limits_checkbox.isChecked():
        readdata.set_maxmin(self,z,'onepanel')
    
        for n in range(start,stop,10):#365         
            self.ax20.plot(z[n][self.nysedmin-30:],
                  self.depth_sed[self.nysedmin-30:],
                  self.spr_aut, alpha = self.a_w,
                  linewidth = self.linewidth, zorder = 8)                          
        
        if self.fielddata_checkbox.isChecked():
            from read_field import field_1p
            #array = readdata_qmain.ReadVar(
            #    self.filename,index,start,stop)
            index = str(self.qlistwidget.currentItem().text())
            field_1p(index,self.ax20)    
